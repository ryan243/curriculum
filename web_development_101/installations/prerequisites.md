...
While your VM is running, do the following steps:
  ...
  6. Type this into the terminal: `sudo apt install linux-headers-$(uname -r) build-essential dkms`. Enter your password when it asks you to. _(__note__: Your password will not be visible in the terminal. This is a security feature to protect your password. Press `Enter` when done.)_
  7. If you get the following errors: **Unable to locate package build-essential** and **Unable to locate package dkms**, type in the following: `sudo apt-get install build-essential` and enter your password. Otherwise, move on to Step 8.
  ...
  13. In the newly opened terminal window, type `sudo ./VBoxLinuxAdditions.run` and hit enter. You will know it is finished when it asks you to close the window.
